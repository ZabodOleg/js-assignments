...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    return Math.acos((x1*x2+y1*y2)/(Math.sqrt(Math.pow(x1,2)+Math.sqrt(Math.pow(y1,2)))*Math.sqrt(Math.pow(x2,2)+Math.sqrt(Math.pow(y2,2)))));
}
...
function getParallelipidedDiagonal(a,b,c) {
    return  Math.sqrt(Math.pow(c,2)+Math.pow(Math.sqrt(Math.pow(a,2)+Math.pow(b,2)),2));
}
/**
...
function toNumber(value, def) {
    ...
    return (typeof value) == "number" ? Number(value):Number(def);
}
module.exports = {
